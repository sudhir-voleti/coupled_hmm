#!/usr/bin/env python3
"""
BEMMAOR HMM with Gamma spend emissions.
Dual observations: y (Poisson count, timing) and z (continuous spend | y>0).
Based on working JRSSC Gamma code, adapted to coupled_hmm structure.
"""
...
def make_bemmaor_gamma(data, K, pilot=False, prior_log_alpha_gamma_mu=0.0, prior_log_alpha_gamma_sigma=1.0):
    """
    BEMMAOR with dual observations:
    y: Poisson count (timing, zero-inflated via NBD)
    z: Continuous spend (Gamma, observed only where y>0)
    ...
    if pilot:
        print("[PILOT] BEMMAOR-Gamma dual: N={}, T={}, K={}".format(N, T, K))
    ...
    with pm.Model(coords={
        ...
        if K == 1:
            ...
            # Gamma log-density: log f(z) = (alpha-1)*log(z) - beta*z + alpha*log(beta) - gammaln(alpha)
...
def run_smc(data, K, draws, chains, seed, out_dir,
    ...
    with make_bemmaor_gamma(data, K,
        ...
        pkl_path = out_dir / "smc_K{}_BEMMAOR_Gamma_dual_N{}_T{}_D{}.pkl".format(
...
def main():
    ...
    parser = argparse.ArgumentParser(description='BEMMAOR-Gamma dual obs')
