#!/usr/bin/env python3
"""
BEMMAOR HMM with Gamma spend emissions and GLOBAL theta (shared across all customers).
No individual heterogeneity - tests whether shared latent factor suffices.
"""
...
def make_bemmaor_gamma_global(data, K, pilot=False, prior_log_alpha_gamma_mu=0.0, prior_log_alpha_gamma_sigma=1.0):
    """
    BEMMAOR-Gamma with GLOBAL theta (scalar, shared across all customers).
    No individual heterogeneity - tests shared latent factor sufficiency.
    """
    y = data['y']
    z = data['z']
    mask = data['mask']
    ...
    if pilot:
        print("[PILOT] BEMMAOR-Gamma global: N={}, T={}, K={}".format(N, T, K))
    ...
    with pm.Model(coords={
        ...
        # =====================================================================
        # 2. SHARED LATENT FACTOR (GLOBAL - scalar, not per-customer)
        # =====================================================================
        theta = pm.Normal("theta", mu=0, sigma=1)  # SCALAR, shared across all customers
        ...
        if K == 1:
            ...
            log_lam = alpha_h[None, None, :] + gamma_h * theta  # theta is scalar, broadcast to all

        lam = pt.exp(pt.clip(log_lam, -5, 5))
        ...
        # =====================================================================
        # 4. GAMMA PART (Spend / z, only where y>0)
        # =====================================================================
        if K == 1:
        ...
        if K == 1:
            ...
            log_mu = beta_m[None, None, :] + gamma_m * theta  # theta scalar, broadcast

        mu = pt.exp(pt.clip(log_mu, -5, 5))
        beta_gamma = alpha_gamma / mu

        # =====================================================================
        # 5. DUAL EMISSION LIKELIHOOD (GAMMA)
        # =====================================================================
        if K == 1:
...
def run_smc(data, K, draws, chains, seed, out_dir,
    ...
    with make_bemmaor_gamma_global(data, K,
        ...
        if 'true_states' in data and data['true_states'] is not None:
            ...
            # Compute state ARI if alpha_filtered available
            try:
                if 'idata' in dir() and hasattr(idata, 'posterior') and 'alpha_filtered' in idata.posterior:
                    alpha = idata.posterior['alpha_filtered'].mean(dim=['chain', 'draw']).values
                    Gamma_est = idata.posterior['Gamma'].mean(dim=['chain', 'draw']).values
                    pred_states = viterbi_decode(alpha, Gamma_est)
                    S_true_arr = np.array(data['true_states'])
                    recovery['state_ari'] = float(compute_ari(S_true_arr, pred_states))
                else:
                    recovery['state_ari'] = np.nan
            except:
                recovery['state_ari'] = np.nan
        ...
        res = {
            'meta': {
                'model_type': 'BEMMAOR-Gamma-global',
                'K': K,
                'N': data['N'],
                'T': data['T'],
        ...
        pkl_path = out_dir / "smc_K{}_BEMMAOR_Gamma_global_N{}_T{}_D{}.pkl".format(
...
def main():
    ...
    parser = argparse.ArgumentParser(description='BEMMAOR-Gamma global theta')
