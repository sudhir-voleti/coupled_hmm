#!/usr/bin/env python3
"""
Hurdle HMM with Gamma spend emissions.
ORDERED VERSION: Reference-state anchoring + ordered deviations.
"""
...
def forward_algorithm_scan(log_emission, log_Gamma, pi0):
    ...
    log_alpha_norm_full = pt.concatenate([
        log_alpha_norm_init[None, :, :], log_alpha_norm_seq], axis=0)
    log_marginal = log_Z_init.squeeze() + pt.sum(log_Z_seq.squeeze(), axis=0)
    return log_marginal, log_alpha_norm_full


def make_hurdle_gamma_ordered(data, K, pilot=False,
                               prior_log_alpha_gamma_mu=0.0,
                               prior_log_alpha_gamma_sigma=1.0):
    y = data['y']
    z = data['z']
    mask = data['mask']
    N, T = data['N'], data['T']
    ...
    if pilot:
        print("[PILOT] HURDLE-Gamma-ORDERED: N={}, T={}, K={}".format(N, T, K))
    ...
    with pm.Model(coords={
        ...
        # NBD PART - REFERENCE STATE + ORDERED
        if K == 1:
            alpha_h = pt.as_tensor_variable(np.array([0.0], dtype=np.float32))
            log_r_raw = pm.Normal("log_r_raw", 0, 1)
            log_r = log_r_raw
        else:
            alpha_h_raw = pm.Normal("alpha_h_raw", 0, 1, shape=K - 1)
            alpha_h_sorted = pt.sort(alpha_h_raw)
            alpha_h = pm.Deterministic(
                "alpha_h",
                pt.concatenate([
                    pt.as_tensor_variable(np.array([0.0], dtype=np.float32)),
                    alpha_h_sorted]))
            log_r_raw = pm.Normal("log_r_raw", 0, 1, shape=K)
            log_r = pm.Deterministic("log_r", log_r_raw)

        r_nbd = pt.exp(log_r)
        log_lam = alpha_h[None, None, :] if K > 1 else alpha_h
        ...
        # GAMMA PART - REFERENCE STATE + ORDERED
        if K == 1:
            beta_m = pt.as_tensor_variable(np.array([0.0], dtype=np.float32))
            log_alpha_gamma_raw = pm.Normal("log_alpha_gamma_raw",
                                             prior_log_alpha_gamma_mu,
                                             prior_log_alpha_gamma_sigma)
            log_alpha_gamma = log_alpha_gamma_raw
        else:
            beta_m_raw = pm.Normal("beta_m_raw", 0, 1, shape=K - 1)
            beta_m_sorted = pt.sort(beta_m_raw)
            ...
            log_alpha_gamma_raw = pm.Normal("log_alpha_gamma_raw",
                                             prior_log_alpha_gamma_mu,
                                             prior_log_alpha_gamma_sigma, shape=K)
            log_alpha_gamma = pm.Deterministic("log_alpha_gamma", log_alpha_gamma_raw)

        alpha_gamma = pt.exp(log_alpha_gamma)
        log_mu = beta_m[None, None, :] if K > 1 else beta_m
        mu = pt.exp(pt.clip(log_mu, -10, 10))
        beta_gamma = alpha_gamma / mu

        # DUAL EMISSION LIKELIHOOD (GAMMA)
        if K == 1:
            log_zero = log_p_zero_nbd
            z_clipped = pt.clip(z, 1e-10, 1e10)
            log_gamma_density = ((alpha_gamma - 1) * pt.log(z_clipped) -
                                 beta_gamma * z_clipped +
                                 alpha_gamma * pt.log(beta_gamma) -
                                 pt.gammaln(alpha_gamma))
            log_p_pos = pt.log1p(-pt.exp(log_zero) + 1e-10)
            log_pos = log_p_pos + log_gamma_density
            ...
            beta_exp = beta_gamma
            log_gamma_density = ((alpha_exp - 1) * pt.log(z_clipped) -
                                 beta_exp * z_exp +
...
def run_smc(data, K, draws, chains, seed, out_dir,
            prior_log_alpha_gamma_mu=0.0, prior_log_alpha_gamma_sigma=1.0):
    cores = min(chains, 4)
    t0 = time.time()

    with make_hurdle_gamma_ordered(data, K,
                                    prior_log_alpha_gamma_mu=prior_log_alpha_gamma_mu,
                                    prior_log_alpha_gamma_sigma=prior_log_alpha_gamma_sigma) as model:
        print("\nHURDLE-Gamma-ORDERED: K={}, N={}, T={}".format(K, data['N'], data['T']))
        print("SMC: draws={}, chains={}, cores={}".format(draws, chains, cores))
        print("ORDERED: alpha_h[0]=0, beta_m[0]=0, deviations sorted")
        ...
        try:
            ...
            ess_by_var = {str(v): float(ess[v].values.min()) 
                          for v in ess.data_vars if hasattr(ess[v].values, 'size')}
        ...
        res = {
            'meta': {
                'model_type': 'HURDLE-Gamma-ORDERED',
                'K': K, 'N': data['N'], 'T': data['T'],
                'world': data.get('world', 'unknown'),
                'draws': draws, 'chains': chains, 'seed': seed,
                'timestamp': datetime.now().isoformat(),
            },
            'diagnostics': {
                'ess_min': ess_min, 'ess_by_var': ess_by_var,
                'rhat_max': rhat_max, 'log_evidence': log_ev, 'time_min': elapsed,
            },
            'recovery': recovery,
            'predictive': {}, 'bdt': {},
            'data_ref': {
        ...
        pkl_path = out_dir / "smc_K{}_HURDLE_Gamma_ORDERED_N{}_T{}_D{}.pkl".format(
...
def main():
    ...
    parser = argparse.ArgumentParser(description='HURDLE-Gamma ORDERED')
    ...
    pkl_path, res, idata = run_smc(
        ...
        prior_log_alpha_gamma_mu=args.prior_log_alpha_gamma_mu,
        prior_log_alpha_gamma_sigma=args.prior_log_alpha_gamma_sigma)
