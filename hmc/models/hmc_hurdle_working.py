#!/usr/bin/env python3
"""
FILE: model_hurdle_27may.py
CREATED: 2026-05-27
CHAT: jmr_27mayv1
STATUS: Hurdle with independent REs + r_k free per state
PYMC: 5.26.1, ARVIZ: 0.22.0
PURPOSE: Zero-coupling baseline with non-structural heterogeneity
...
def build_model(Y_timing, Y_spend, K=3):
    ...
    with pm.Model() as model:
        ...
        # NBD for timing (independent)
        lam = pt.exp(alpha_h[None, None, :] + u[:, :, None])
...
def compute_ari_forward_filter(dgp, idata):
    ...
    for i in range(N):
        ...
        for t in range(T):
            ...
            for k in range(K):
                # NBD timing with u_i
                lam = np.exp(alpha_h_est[k] + u_est[i])
                lam = max(lam, 1e-10)
...
def main():
    ...
    print('Fitting Hurdle (r-free) on ' + dgp['world'] + ' world...')
    t0 = time.time()
    ...
    try:
        ...
        pkl_name = 'fit_Hurdle-rfree_' + dgp['world'] + '_N' + str(dgp['N']) + '_T' + str(dgp['T']) + '.pkl'
