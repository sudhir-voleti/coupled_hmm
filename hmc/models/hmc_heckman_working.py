#!/usr/bin/env python3
"""
FILE: model_heckman_27may.py
CREATED: 2026-05-27
CHAT: jmr_27mayv1
STATUS: Heckman with probit timing + Gamma spend + rho statistical coupling
PYMC: 5.26.1, ARVIZ: 0.22.0
PURPOSE: Statistical coupling benchmark via BVN latent threshold
MATCHES: dgp_rfree_27may_working.py
NEXT: Run in parallel with BEMMAOR variants
"""
import numpy as np
import pymc as pm
...
def build_model(Y_timing, Y_spend, K=3):
    ...
    with pm.Model() as model:
        ...
        # Heckman: correlated random effects (not shared theta)
        sigma_u = pm.HalfNormal('sigma_u', sigma=1.0)
        sigma_v = pm.HalfNormal('sigma_v', sigma=1.0)
        rho = pm.Uniform('rho', lower=-0.5, upper=0.5)

        u = pm.Normal('u', mu=0, sigma=sigma_u, shape=(N, 1))
        v_mean = rho * (sigma_v / sigma_u) * u
        v_sd = sigma_v * pt.sqrt(1 - rho**2)
        v = pm.Normal('v', mu=v_mean, sigma=v_sd, shape=(N, 1))

        # No gamma_m (no structural coupling)
        gamma_m = 0.0
        ...
        # Probit for timing (selection)
        probit_timing = alpha_h[None, None, :] + u[:, :, None]
        log_p_zero = pt.log(pt.erfc(probit_timing / pt.sqrt(2)) / 2 + 1e-10)
        log_p_pos = pt.log1p(-pt.exp(log_p_zero) + 1e-10)

        # Gamma for spend (outcome)
        mu_spend = pt.exp(beta_m[None, None, :] + v[:, :, None])
        ...
        log_emission = pt.where(pt.eq(y_exp, 0), log_p_zero, log_p_pos + log_gamma)
...
def compute_ari_forward_filter(dgp, idata):
    ...
    if 'u' in post:
        u_est = post['u'].mean(dim=['chain', 'draw']).values.squeeze()
    else:
        u_est = np.zeros(dgp['N'])
    ...
    for i in range(N):
        ...
        for t in range(T):
            ...
            for k in range(K):
                # Probit timing
                probit_val = alpha_h_est[k] + u_est[i]
                p_zero = 0.5 * math.erfc(probit_val / math.sqrt(2))
                ...
                # Gamma spend (no v in ARI - use population mean)
...
def extract_metrics(idata, dgp):
    ...
    if 'rho' in post:
        ...
        metrics['rho_rec'] = rho_est / rho_true if rho_true != 0 else rho_est
        metrics['rho_mae'] = abs(rho_est - rho_true)
    else:
        metrics['rho_rec'] = np.nan
        metrics['rho_mae'] = np.nan
...
def main():
    ...
    print('Fitting Heckman (r-free) on ' + dgp['world'] + ' world...')
    t0 = time.time()
    ...
    try:
        ...
        pkl_name = 'fit_Heckman-rfree_' + dgp['world'] + '_N' + str(dgp['N']) + '_T' + str(dgp['T']) + '.pkl'
        ...
        print('rho_rec:    ' + str(round(metrics['rho_rec'], 3)))
